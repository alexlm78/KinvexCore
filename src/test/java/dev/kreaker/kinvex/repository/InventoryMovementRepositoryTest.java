...
class InventoryMovementRepositoryTest {
    ...
    void findMovementsBetween_ShouldReturnMovementsInDateRange() {
        ...
        InventoryMovement movement =
                new InventoryMovement(product, InventoryMovement.MovementType.IN, 10);
        ...
        List<InventoryMovement> movements =
                inventoryMovementRepository.findMovementsBetween(startDate, endDate);
    ...
    void findProductMovementSummaryBetween_ShouldReturnCorrectSummary() {
        ...
        InventoryMovement inMovement =
                new InventoryMovement(product, InventoryMovement.MovementType.IN, 20);
        inMovement.setCreatedBy(user);
        entityManager.persistAndFlush(inMovement);

        // Create outbound movement
        InventoryMovement outMovement =
                new InventoryMovement(product, InventoryMovement.MovementType.OUT, 5);
        outMovement.setCreatedBy(user);
        entityManager.persistAndFlush(outMovement);

        // When
        List<Object[]> summary =
                inventoryMovementRepository.findProductMovementSummaryBetween(startDate, endDate);
