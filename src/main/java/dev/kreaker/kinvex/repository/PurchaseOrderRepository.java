...
public interface PurchaseOrderRepository extends JpaRepository<PurchaseOrder, Long> {
    ...
    Page<PurchaseOrder> findBySupplierAndStatus(Supplier supplier, OrderStatus status, Pageable pageable);
    ...
    // Overdue orders
    @Query("SELECT po FROM PurchaseOrder po WHERE po.expectedDate < CURRENT_DATE AND po.status IN ('PENDING', 'CONFIRMED', 'PARTIAL')")
    List<PurchaseOrder> findOverdueOrders();

    @Query("SELECT po FROM PurchaseOrder po WHERE po.expectedDate BETWEEN CURRENT_DATE AND :futureDate AND po.status IN ('PENDING', 'CONFIRMED')")
    ...
    List<PurchaseOrder> findOrdersCreatedBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT po.status, COUNT(po), SUM(po.totalAmount) "
            + "FROM PurchaseOrder po "
            + "WHERE po.createdAt BETWEEN :startDate AND :endDate "
            + "GROUP BY po.status")
    List<Object[]> findOrderStatisticsBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT MONTH(po.orderDate), YEAR(po.orderDate), COUNT(po), SUM(po.totalAmount) "
            + "FROM PurchaseOrder po "
    ...
    List<Object[]> findMonthlyOrderStatistics(@Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT po.supplier, COUNT(po), SUM(po.totalAmount), AVG(po.totalAmount) "
            + "FROM PurchaseOrder po "
            + "WHERE po.createdAt BETWEEN :startDate AND :endDate "
            + "GROUP BY po.supplier ORDER BY SUM(po.totalAmount) DESC")
    List<Object[]> findOrdersBySupplierBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT COUNT(po) FROM PurchaseOrder po WHERE po.status = :status")
    long countByStatus(@Param("status") OrderStatus status);

    @Query("SELECT SUM(po.totalAmount) FROM PurchaseOrder po WHERE po.status = 'COMPLETED' AND po.receivedDate BETWEEN :startDate AND :endDate")
    BigDecimal sumCompletedOrdersAmountBetween(@Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT AVG(CAST((po.receivedDate - po.orderDate) AS int)) "
            + "FROM PurchaseOrder po "
            + "WHERE po.status = 'COMPLETED' AND po.receivedDate IS NOT NULL "
            + "AND po.receivedDate BETWEEN :startDate AND :endDate")
    Double findAverageDeliveryTimeBetween(@Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);
}
