...
public interface SupplierRepository extends JpaRepository<Supplier, Long> {
    ...
    @Query(
            "SELECT s, COUNT(po), AVG(po.totalAmount), "
    ...
    List<Object[]> findSupplierPerformanceBetween(
    ...
    @Query(
            "SELECT s, COUNT(po), SUM(po.totalAmount) "
                    + "FROM Supplier s LEFT JOIN s.purchaseOrders po "
                    + "WHERE po.status = 'COMPLETED' AND po.createdAt BETWEEN :startDate AND :endDate "
                    + "GROUP BY s ORDER BY SUM(po.totalAmount) DESC")
    List<Object[]> findTopSuppliersByVolumeBetween(
            @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
