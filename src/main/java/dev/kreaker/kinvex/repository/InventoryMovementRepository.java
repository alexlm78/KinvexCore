...
public interface InventoryMovementRepository extends JpaRepository<InventoryMovement, Long> {
    ...
    Page<InventoryMovement> findByCreatedAtBetween(LocalDateTime startDate, LocalDateTime endDate, Pageable pageable);

    List<InventoryMovement> findByProductAndCreatedAtBetween(Product product, LocalDateTime startDate, LocalDateTime endDate);
    ...
    List<InventoryMovement> findMovementsBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT im.product, SUM(CASE WHEN im.movementType = 'IN' THEN im.quantity ELSE 0 END) as inbound, "
    ...
    @Query("SELECT im.movementType, im.referenceType, COUNT(im), SUM(im.quantity) "
            + "FROM InventoryMovement im "
            + "WHERE im.createdAt BETWEEN :startDate AND :endDate "
            + "GROUP BY im.movementType, im.referenceType")
    List<Object[]> findMovementStatisticsByTypeBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT DATE(im.createdAt), SUM(CASE WHEN im.movementType = 'IN' THEN im.quantity ELSE 0 END), "
            + "SUM(CASE WHEN im.movementType = 'OUT' THEN im.quantity ELSE 0 END) "
            + "FROM InventoryMovement im "
            + "WHERE im.createdAt BETWEEN :startDate AND :endDate "
            + "GROUP BY DATE(im.createdAt) ORDER BY DATE(im.createdAt)")
    List<Object[]> findDailyMovementSummaryBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT im.sourceSystem, COUNT(im), SUM(im.quantity) "
            + "FROM InventoryMovement im "
    ...
    List<Object[]> findOutboundMovementsBySourceBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT im.product, SUM(im.quantity) "
            + "FROM InventoryMovement im "
            + "WHERE im.movementType = 'OUT' AND im.createdAt BETWEEN :startDate AND :endDate "
            + "GROUP BY im.product ORDER BY SUM(im.quantity) DESC")
    List<Object[]> findMostSoldProductsBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);
    ...
    @Query("SELECT im.product, SUM(im.quantity) "
            ...
            + "GROUP BY im.product ORDER BY SUM(im.quantity) DESC")
    List<Object[]> findMostReceivedProductsBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);
    ...
    @Query("SELECT SUM(im.quantity) FROM InventoryMovement im WHERE im.movementType = :movementType AND im.createdAt BETWEEN :startDate AND :endDate")
    Long sumQuantityByMovementTypeBetween(@Param("movementType") MovementType movementType,
            @Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT COUNT(DISTINCT im.product) FROM InventoryMovement im WHERE im.createdAt BETWEEN :startDate AND :endDate")
    long countDistinctProductsWithMovementsBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);
}
