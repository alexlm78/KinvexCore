...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Long> {
    ...
    @Query("SELECT od.product, SUM(od.quantityOrdered), SUM(od.quantityReceived), SUM(od.totalPrice) "
            + "FROM OrderDetail od "
            + "JOIN od.order po "
            + "WHERE po.createdAt BETWEEN :startDate AND :endDate "
            + "GROUP BY od.product ORDER BY SUM(od.quantityOrdered) DESC")
    List<Object[]> findProductOrderStatisticsBetween(@Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("SELECT od.product, SUM(od.quantityReceived), AVG(od.unitPrice) "
            + "FROM OrderDetail od "
            + "JOIN od.order po "
    ...
    @Query("SELECT SUM(od.quantityOrdered - od.quantityReceived) "
            + "FROM OrderDetail od "
    ...
    @Query("SELECT od.product, SUM(od.quantityOrdered - od.quantityReceived) "
            + "FROM OrderDetail od "
    ...
    @Query("SELECT SUM(od.totalPrice) FROM OrderDetail od JOIN od.order po WHERE po.status = 'COMPLETED'")
